...
pub struct UrlSet {
    ...
    ///
    /// This is relevant for adding links with `rel=alternate` or `rel=canonical`.
...
impl UrlSet {
    ...
    pub fn new(urls: Vec<Url>) -> Result<Self, UrlSetError> {
        ...
        for url in &urls {
            ...
            // if any <url>s exist that contain a canonical, set the xhtml namespace
            if url.canonical.is_some() && xmlns_xhtml.is_none() {
                xmlns_xhtml = Some(XHTML_NAMESPACE.to_string());
            }
        }
